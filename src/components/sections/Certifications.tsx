...
const normalizeLogoPath = (path: string): string => {
  if (!path) return '';
  ...
  // Automatically strip leading 'public/' if user entered it like 'public/logos/...'
...
export const Certifications: React.FC = () => {
  ...
  const totalPages = Math.ceil(certificationsData.length / CARDS_PER_PAGE);
  const startIdx = page * CARDS_PER_PAGE;
  const currentCerts = certificationsData.slice(startIdx, startIdx + CARDS_PER_PAGE);
  ...
  const prev = () => goTo(page > 0 ? page - 1 : totalPages - 1, 'prev');
  const next = () => goTo(page < totalPages - 1 ? page + 1 : 0, 'next');
  ...
  const renderLogo = (cert: Certification) => {
    const customLogo = cert.logo || cert.logoUrl || cert.icon;

    if (customLogo && !failedLogos[cert.id]) {
      const src = normalizeLogoPath(customLogo);
      return (
        <img
          src={src}
          ...
          loading="lazy"
          onError={() => {
            setFailedLogos((prev) => ({ ...prev, [cert.id]: true }));
          }}
        />
      );
    }

    return getCompanyLogo(cert.issuer, cert.title);
  };
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-8 md:px-12 relative z-10">
        ...
        {/* Cards — paginated with fade+slide animation */}
        <div
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 auto-rows-fr gap-3.5 sm:gap-4 lg:gap-[clamp(0.875rem,1.1vw,1.125rem)]"
          style={{
        ...
        >
          {currentCerts.map((cert) => (
            <div
              key={cert.id}
              className="relative rounded-2xl bg-white/[0.03] border border-white/10 ring-1 ring-white/5 backdrop-blur-md p-4 sm:p-[clamp(1rem,1.1vw,1.25rem)] flex flex-col justify-between shadow-lg overflow-hidden h-full min-h-[195px] sm:min-h-[205px]"
            >
              <div className="flex flex-col h-full justify-between">
                <div>
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="w-8 h-8 sm:w-8.5 sm:h-8.5 rounded-xl bg-white/[0.06] border border-white/15 flex items-center justify-center p-1.5 shadow-sm shrink-0 overflow-hidden">
                        {renderLogo(cert as Certification)}
                      </div>
                      <div className="min-w-0">
                        <div className="text-[10px] font-mono text-white/50 uppercase tracking-wider truncate">{cert.issuer}</div>
                        <div className="text-[11px] sm:text-xs font-medium text-white/80 truncate">{cert.category}</div>
                      </div>
                    </div>
                    {cert.credentialUrl && (
                      <a
                  ...
                  <h3
                    className="font-haas font-bold text-[14.5px] sm:text-[15px] xl:text-[16px] text-white tracking-tight leading-snug mt-3 sm:mt-3.5 line-clamp-2 min-h-[2.75rem] sm:min-h-[2.9rem] flex items-start"
                    title={cert.title}
                  >
                    {cert.title}
                  </h3>
                  <p
                    className="text-[12px] sm:text-[12.5px] xl:text-[13px] text-white/60 leading-relaxed font-normal mt-1.5 sm:mt-2 line-clamp-2 min-h-[2.6rem] sm:min-h-[2.8rem]"
        ...
        {/* Navigation Controls: Arrows + Indicator Dots (matching Project Archive CardStack exactly) */}
        {totalPages > 1 && (
            ...
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 z-20">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={animating}
                  className="w-10 h-10 rounded-full bg-white/[0.06] hover:bg-white/15 border border-white/15 text-white/80 hover:text-white flex items-center justify-center transition-all active:scale-95 shadow-sm disabled:opacity-40"
                >
