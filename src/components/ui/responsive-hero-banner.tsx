...
export const ResponsiveHeroBanner: React.FC<ResponsiveHeroBannerProps> = ({
  ...
  return (
    <section className="w-full max-w-full isolate min-h-[100svh] overflow-hidden relative flex flex-col justify-between pt-24 sm:pt-28 pb-10 bg-[#0A0A0A]">
      ...
      {/* Top Header Scrim */}
      <div className="pointer-events-none absolute top-0 inset-x-0 h-28 bg-gradient-to-b from-black/70 to-transparent" />

      {/* Bottom Soft Edge Dissolve into Next Section */}
      <div className="pointer-events-none absolute bottom-0 inset-x-0 h-36 sm:h-48 bg-gradient-to-t from-[#0A0A0A] via-[#0A0A0A]/75 to-transparent" />

      {/* Main Center Content */}
      <div className="z-10 relative my-auto py-8 px-6 w-full max-w-5xl mx-auto text-center">
        {/* Status Badge */}
        <div className="mb-6 inline-flex items-center gap-2 sm:gap-2.5 rounded-full bg-white/10 p-1 pr-3.5 sm:pr-4 ring-1 ring-white/15 backdrop-blur-md animate-fade-slide-in-1 max-w-full shadow-sm">
        ...
        <div className="animate-fade-slide-in-2 w-full max-w-5xl mx-auto flex items-center justify-center">
          <WarpText
            ...
            lineHeight={0.93}
            style={{ height: 'clamp(130px, 18vw, 235px)' }}
            className="w-full"
          />
        </div>
        ...
        <BlurText
          ...
          animateBy="words"
          direction="top"
          className="text-[16px] sm:text-[18px] md:text-[20px] text-white/75 max-w-2xl mt-3 sm:mt-4 mx-auto leading-relaxed text-center"
        />

        {/* Trusted Clients Social Proof (Above CTAs without any blur background) */}
        {showTrustedClients && (
          <div className="mt-5 sm:mt-7 flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3.5 animate-fade-slide-in-3 text-center px-4 w-full">
        ...
        {/* Action CTAs */}
        <div className="flex flex-col sm:flex-row sm:gap-4 mt-6 sm:mt-8 gap-3 items-center justify-center animate-fade-slide-in-4 w-full max-w-md sm:max-w-none mx-auto px-4">
          <a
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 text-sm font-medium text-neutral-950 bg-white hover:bg-white/90 rounded-full py-3.5 px-6 transition-all shadow-lg hover:scale-105 active:scale-95"
            data-cursor="link"
          >
            {primaryButtonText}
          ...
          <a
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-full bg-white/10 hover:bg-white/15 ring-1 ring-white/15 px-6 py-3.5 text-sm font-medium text-white transition-colors"
            data-cursor="link"
          >
            {secondaryButtonText}
      ...
      <div className="z-10 relative px-4 sm:px-6 max-w-5xl mx-auto w-full pb-2 overflow-hidden">
        <p className="animate-fade-slide-in-1 text-xs text-white/50 text-center uppercase tracking-widest font-medium">
          {partnersTitle}
        </p>
        {showToolDock && toolDockItems && toolDockItems.length > 0 ? (
          ...
          <div className="flex flex-wrap items-center justify-center gap-2.5 sm:gap-3 mt-3 animate-fade-slide-in-2">
