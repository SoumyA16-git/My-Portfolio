...
export function CardStack<T extends CardStackItem>({
  ...
  // On mobile screen, limit visible side cards to 3 total (1 left, 1 center, 1 right)
        ...
        >
          <AnimatePresence initial={false}>
            {items.map((item, i) => {
              ...
              // Deep Half-Round / U-Shaped Architectural Dome Curve
              // Keeps the deck inside section width while plunging sides down into a deep arc
              const stepAngle = isMobile ? 12 : (spreadDeg * 0.515);
              const angleRad = (off * stepAngle * Math.PI) / 180;
              const archRadiusX = isMobile ? responsiveWidth * 0.42 : Math.min(420, responsiveWidth * (1.6 - overlap));
              const archRadiusY = isMobile ? 35 : 90;

              const x = archRadiusX * Math.sin(angleRad);
              const y = archRadiusY * (1 - Math.cos(angleRad));
              const rotateZ = off * (isMobile ? 7 : (spreadDeg * 0.297));
              const z = -abs * (isMobile ? 22 : depthPx);
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-3xl overflow-hidden shadow-2xl border border-white/10 ring-1 ring-white/5 bg-neutral-950",
...
function DefaultFanCard({ item, active }: { item: CardStackItem; active: boolean }) {
  return (
    <div className="relative h-full w-full flex flex-col justify-between overflow-hidden bg-neutral-950">
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/95 via-black/60 to-black/25" />

      {/* Top Metadata Header Strip */}
      <div className="relative z-10 p-4 sm:p-6 flex items-center justify-between gap-2">
        {item.category && (
          <span className="px-3 py-1 rounded-full bg-black/70 border border-white/20 backdrop-blur-md text-[10.5px] sm:text-[11px] font-medium text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] shadow-sm">
            {item.category}
          </span>
        )}
        {item.year && (
          <span className="px-2.5 py-0.5 rounded-full bg-black/60 border border-white/15 font-mono text-[10.5px] sm:text-[11px] text-white/80 backdrop-blur-md drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] shadow-sm">
            {item.year}
          </span>
        )}
      </div>
      ...
      <div className="relative z-10 p-4 sm:p-6 flex flex-col justify-end space-y-2.5 sm:space-y-3">
        <div>
          <h3 className="text-lg sm:text-2xl font-haas font-bold text-white tracking-tight leading-snug drop-shadow-[0_2px_10px_rgba(0,0,0,0.95)]">
            {item.title}
          </h3>
          {item.description && (
            <p className="mt-1 line-clamp-2 text-xs sm:text-sm text-white/90 leading-relaxed font-normal drop-shadow-[0_2px_8px_rgba(0,0,0,0.9)]">
              {item.description}
            </p>
          )}
        </div>
        ...
        {item.tools && item.tools.length > 0 && (
          <div className="flex flex-wrap gap-1.5 pt-0.5">
            {item.tools.slice(0, 3).map((tool, idx) => (
              <span
                key={idx}
                className="text-[10px] sm:text-[10.5px] font-mono text-white/90 bg-black/60 border border-white/20 px-2 py-0.5 rounded-md backdrop-blur-md drop-shadow-[0_2px_4px_rgba(0,0,0,0.85)] shadow-sm"
              >
                {tool}
              </span>
            ))}
