...
const buildTextCanvas = ({
  ...
  probe.className = 'font-haas font-bold';
  Object.assign(probe.style, {
    ...
    fontFamily: props.fontFamily && props.fontFamily !== 'inherit' 
      ? props.fontFamily 
      : "'Haas Grot Text R Web', 'Haas Grot Text R', 'Neue Haas Grotesk Text Pro', 'Helvetica Neue', Arial, sans-serif",
  ...
  const fontFamily = (rawFontFamily && !rawFontFamily.includes('var('))
    ? rawFontFamily
    : "'Haas Grot Text R Web', 'Haas Grot Text R', 'Neue Haas Grotesk Text Pro', 'Helvetica Neue', Arial, sans-serif";
  const fontWeight = computed.fontWeight || String(props.fontWeight || 700);
  let letterSpacing = computed.letterSpacing === 'normal' ? -2.5 : parseFloat(computed.letterSpacing) || -2.5;
  let lineHeight = parseFloat(computed.lineHeight);
  if (!Number.isFinite(lineHeight)) {
    lineHeight = fontSizePx * (typeof props.lineHeight === 'number' ? props.lineHeight : 0.93);
  }
  probe.remove();
...
const syncUniforms = (program: Program, props: BuildProps) => {
...
export const WarpText: React.FC<WarpTextProps> = ({
  ...
  const propsRef = useRef<BuildProps>({
    text,
    color,
    fontSize,
    fontWeight,
