...
const siteData = JSON.parse(fs.readFileSync(siteJsonPath, 'utf8').replace(/^\uFEFF/, ''));
...
const projects = JSON.parse(fs.readFileSync(projectsJsonPath, 'utf8').replace(/^\uFEFF/, ''));
...
projects.forEach((proj, idx) => {
  ...
  if (!proj.images?.thumbnail?.src) {
    console.error(`❌ Error in project "${proj.slug}": Missing thumbnail image.`);
    hasErrors = true;
  }
});
